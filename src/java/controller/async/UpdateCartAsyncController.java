 ...
 * @author toten
 */
@WebServlet(name = "UpdateCart", urlPatterns = {"/update-quantity-async"})
public class UpdateCartAsyncController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            double totalPrice = carts.get(productId).getProduct().getPrice() * carts.get(productId).getQuantity();
            ...
            totalAmount = Math.ceil(totalAmount * 100.0) / (100.0);
            request.setAttribute("totalAmount", Math.ceil(totalAmount * 100.0) / (100.0));
            request.setAttribute("carts", carts);

            out.print(quantity+"|"+totalPrice+"|"+totalAmount);
        }
    }
